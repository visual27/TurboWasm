...
function parseTrailingRepeatPath(
  ...
  if (lastComma < 0) {
    diagnostics.push({
      severity: 'error',
      code: 'gpu.repeat_path_required',
      message: `@repeat directive requires trailing ', repeatPath="<path>"' (got '${truncate(tail, 32)}')`,
      regionId,
      blockId,
      line,
      column: 0,
  ...
  if (!m) {
    ...
    // keeps the original tail and reports the missing attribute below.
    diagnostics.push({
      severity: 'error',
      code: 'gpu.repeat_path_required',
      message: `@repeat directive requires trailing ', repeatPath="<path>"' (got '${truncate(tail, 32)}')`,
...
function parseRepeat(
  ...
  // §Phase 4: trailing `, repeatPath="..."` is REQUIRED.
  ...
  const repeatPath = pathSuffix.repeatPath;
  right = pathSuffix.tail;

  // Reject legacy `blockId=` on `@repeat` even if `repeatPath=` is
  // missing — the parser cannot silently keep the v9 contract.
  ...
  // Phase 4 contract: when the trailing `repeatPath` (or legacy
  // `blockId=`) attribute emits an error diagnostic, return the
  // directive as `null` so the owning region D1-demotes. A missing
  // repeat path is not recoverable — the resolver cannot guess which
  // control_repeat the user meant.
  if (!repeatPath) {
    return {
      directive: null,
      diagnostic: suffixDiagnostics[0] ?? makeDiag(
        `malformed @repeat: missing repeatPath`,
        regionId,
        blockId,
        line,
        0,
