...
// Generated fixture workspace is `test/.test-fixtures/` (gitignored). All
// test fixtures live there and are regenerated by
// `scripts/ensure-test-fixtures.mjs`. Kept under `test/` so tests and
// fixtures travel together for IDE grouping while staying outside of
// `src/` (which would otherwise be picked up by tooling that expects
// production code there).
...
/**
 * Library entry point: write `bench-touching.sb3` into `.test-fixtures/`.
 * Returns the absolute path to the written file (or `null` if the
 * script was invoked directly, in which case `main()` already wrote it).
 */
export async function writeBenchTouchingFixture() {
  mkdirSync(outDir, { recursive: true });
  const outPath = resolve(outDir, 'bench-touching.sb3');
...
async function main() {
  await writeBenchTouchingFixture();
}
